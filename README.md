...
- `app_settings` — настройки марафона для сайта и бота;
- `bot_faq` — база знаний ИИ-помощника;
- `bot_interactions` — журнал вопросов и ответов Telegram-бота.
...
ИИ-помощник работает без платных внешних API: он определяет намерение пользователя по ключевым словам, сначала ищет ответ в таблице `bot_faq`, затем использует встроенную базу знаний. Если уверенного ответа нет, вопрос автоматически превращается в обращение к администратору.
